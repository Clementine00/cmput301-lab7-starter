...
public class MainActivityTest {
    ...
    @Rule
    public IntentsRule intentsRule = new IntentsRule();  // initializes Espresso Intents
    @Rule
...
onData(is(instanceOf(String.class))).inAdapterView(withId(R.id.city_list)).atPosition(0).check(matches((withText("Edmonton"))));
    ...
    public void testActivitySwitch(){
        ...
        // Click on "Edmonton" in the ListView
        onData(allOf(is(instanceOf(String.class)), is("Edmonton"))).perform(click());

        // Verify the new activity was launched
    ...
    public void testCityNameConsistent(){
        //Click on Add City Button
        onView(withId(R.id.button_add)).perform(click());
        // Type "Calgary" in the editText
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Calgary"));
        // Click on Confirm
        onView(withId(R.id.button_confirm)).perform(click());

        // Click on "Calgary" in the ListView
        onData(allOf(is(instanceOf(String.class)), is("Calgary"))).perform(click());
        //Verify City name is the same as in MainActivity
    ...
    public void testBackButton(){
        //Click on Add City Button
        onView(withId(R.id.button_add)).perform(click());
        // Type "Regina" in the editText
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Regina"));
        // Click on Confirm
        onView(withId(R.id.button_confirm)).perform(click());
        // Click on "Calgary" in the ListView
        onData(allOf(is(instanceOf(String.class)), is("Regina"))).perform(click());
        //Click on back button
        onView(withId(R.id.button_back)).perform(click());
        //Check if we are back to displaying the city list
